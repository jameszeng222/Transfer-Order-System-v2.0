...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw('INSERT INTO transfer_orders_new SELECT * FROM transfer_orders');
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw(`
    CREATE TABLE transfer_orders_old (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      transfer_no TEXT UNIQUE NOT NULL,
      outbound_order_no TEXT,
      inbound_order_no TEXT UNIQUE NOT NULL,
      ...
      team TEXT,
      transfer_type TEXT CHECK(transfer_type IN ('DOMESTIC_TO_OVERSEAS', 'OVERSEAS_TO_OVERSEAS', 'RETURN_TO_SHELF', 'FBA_OUTBOUND')),
      status TEXT DEFAULT 'PENDING_OUTBOUND' CHECK(status IN ('PENDING_OUTBOUND', 'OUTBOUNDED', 'IN_TRANSIT', 'RECEIVED', 'SHELVED', 'COMPLETED', 'CANCELLED')),
      total_sku_count INTEGER DEFAULT 0,
      total_qty INTEGER DEFAULT 0,
      total_carton_count INTEGER DEFAULT 0,
      logistics_status TEXT,
  ...
  await knex.raw("INSERT INTO transfer_orders_old SELECT * FROM transfer_orders WHERE status NOT IN ('PARTIAL_SHELVED')");

  await knex.raw('DROP TABLE transfer_orders');
  await knex.raw('ALTER TABLE transfer_orders_old RENAME TO transfer_orders');
